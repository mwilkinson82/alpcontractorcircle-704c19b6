...
async function enrollFromCheckout(session: Record<string, any>) {
  ...
  if (paymentLinkId !== INDIVIDUAL_PAYMENT_LINK && paymentLinkId !== COMPANY_PAYMENT_LINK) {
  ...
  if (!email) throw new Error("Paid Checkout Session did not include an email address.");
  const enrollmentType = paymentLinkId === COMPANY_PAYMENT_LINK ? "company" : "individual";
  ...
  const values = {
    ...
    company_name: session.custom_fields?.find?.((field: any) => field.key === "company_name")?.text?.value || null,
  ...
  const mutation = existing
    ? supabase.from("intensive_enrollments").update(values).eq("id", existing.id).select("*").single()
    : supabase.from("intensive_enrollments").insert({ ...values, access_token: randomToken() }).select("*").single();
...
async function revokeByPaymentIntent(paymentIntentId: string | null, status: "refunded" | "disputed") {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (event.type === "checkout.session.completed" || event.type === "checkout.session.async_payment_succeeded") {
      ...
      result = await revokeByPaymentIntent(asId(object.payment_intent), "refunded");
    } else if (event.type === "charge.dispute.created") {
      result = await revokeByPaymentIntent(asId(object.payment_intent), "disputed");
    }
